...
int  ft_atoi(char *str)
{
  ...
  result = 0;
  sign = 1;
  while (str == 32 || (str >= 9 && str <= 13))   // Skip any leading whitespace characters (' ' space, '\t' tab, '\n' newline, '\v' vertical space, 't' horizontal space, '\f' new page, '\r' carriage return', etc.)
    str++;
